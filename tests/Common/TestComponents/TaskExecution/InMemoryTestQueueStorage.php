...
class InMemoryTestQueueStorage implements TaskQueueStorage
{
    ...
    {
        return key_exists($id, $this->queue) ? $this->queue[$id] : null;
    }
     ...
     * @param array $additionalWhere List of key/value pairs to set in where clause when saving queue item. Key is queue item
     * field and value is condition value for that field.
    ...
    public function findOldestQueuedItems($limit = 10)
    {
        ...
        foreach ($this->getSortedQueue(array('queueTimestamp' => TaskQueueStorage::SORT_ASC)) as $item) {
            if (
                ...
                in_array($item['queueName'], $runningQueues)
            ) {
                continue;
            }
            ...
            if (is_null($queueItemInResult) || $item['queueTimestamp'] < $queueItemInResult->getQueueTimestamp()) {
