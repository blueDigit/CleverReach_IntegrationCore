...
class TaskTest extends TestCase
{
    ...
    public function testItShouldBePossibleToReportThatTasksIsAlive()
    {
        ...
        $task->when(AliveAnnouncedTaskEvent::CLASS_NAME, function (AliveAnnouncedTaskEvent $event) use(&$aliveAnnouncedEvent) {
        ...
        // Assert
        $this->assertNotNull($aliveAnnouncedEvent, 'Task must fire AliveAnnouncedTaskEvent when reporting that it is alive.');
    }
    ...
    public function testItShouldNotBePossibleToReportThatTasksIsAliveTooFrequently()
    {
        ...
        $task->when(AliveAnnouncedTaskEvent::CLASS_NAME, function () use(&$aliveAnnouncedEventCount) {
        ...
        // Assert
        $this->assertSame(1, $aliveAnnouncedEventCount, 'Task must fire AliveAnnouncedTaskEvent only when alive signal frequency time is elapsed.');
    }
    ...
    public function testReportingProgressShouldDeferNextAliveSignal()
    {
        ...
        // Assert
        $this->assertSame(0, $aliveAnnouncedEventCount, 'Reporting progress should defer next AliveAnnouncedTaskEvent.');
    }
    ...
    public function testItShouldBeAbleToReportProgressOnTask()
    {
        ...
        /** @var ProgressedTaskEvent $progressedEvent */
        $progressedEvent = null;
        $task->when(ProgressedTaskEvent::CLASS_NAME, function (ProgressedTaskEvent $event) use(&$progressedEvent) {
